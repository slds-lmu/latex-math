...
\newcommand{\GEhholdouti}[1][i]{\GEh_{\Jtraini[#1], \Jtesti[#1]}(\inducer, \lamv, |\Jtraini[#1]|, \rho)} % GE-hat_{Jtrain_i,Jtest_i} (I, lam, |Jtrain_i|, rho)
...
\newcommand{\GErhoDef}{\lim_{\ntest\rightarrow\infty} \E \left[ \rho\left(\yv_{\Jtest}, \FJtestftrain\right)\right]} % GE formal def
...
\newcommand{\GED}{\GE_{\D}} % Generalization error GE
...
\newcommand{\FJ}[1][J]{\F_{#1}} % predscore mat index vector J 
\newcommand{\FJf}{\FJ[J,f]} % predscore mat index vector J and model f
\newcommand{\FJtestfh}{\FJ[\Jtest, \fh]} % predscore mat index vector Jtest and model f hat
\newcommand{\FJtestftrain}{\F_{\Jtest,\inducer(\Dtrain, \lamv)}} % predscore mat index vector Jtest and model f
\newcommand{\FJtestftraini}[1][i]{\F_{\Jtesti[#1],\inducer(\Dtraini[#1], \lamv)}}  % predscore mat i-th index vector Jtest and model f
\newcommand{\FJfDef}{\left(f(\xv^{(J^{(1)})}),\dots, f(\xv^{(J^{(m)})})\right)} % def of predscore mat index vector J and model f
