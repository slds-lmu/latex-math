...
\newcommand{\obsi}[1][i]{\left(\xv^{(#1)},y^{(#1)}\right)} % observation (x^(i), y^(i))
\newcommand{\Dset}{\left( \obsi[1], \ldots, \obsi[n]\right)} % {(x1,y1)), ..., (xn,yn)}, data
...
\newcommand{\thetat}{\thetab^{[t]}} % theta^[t] in optimization
\newcommand{\thetatn}{\thetab^{[t+1]}} % theta^[t+1] in optimization
\newcommand{\thxh}{\thetab^T \xv} % linear combination with theta
\newcommand{\thetahDnlambda}{\thetabh_{\Dn, \lamv}} %theta learned on Dn with hp lambda
\newcommand{\thetahDlambda}{\thetabh_{\D, \lamv}} %theta learned on D with hp lambda
...
\newcommand{\pikxt}{\pi_k(\xv ~|~ \thetab)} % pi_k(x | theta), P(y = k | x, theta)
